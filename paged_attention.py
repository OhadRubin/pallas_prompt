...
def paged_flash_attention_kernel(
  ...
  """Pallas Kernel for performing attention.

    This kernel is designed to handle the attention mechanism in large
    language models (LLMs) by dividing attention computation into pages
    for more efficient memory management and execution on TPU.

    Args:
        lengths_ref: A reference to the lengths array, [batch_size] array indicating
            the length of the sequence for each example in the batch.
        page_indices_ref: A reference to the page indices array, [batch_size, pages_per_sequence]
            array specifying the page index of the key/value cache.
        buffer_index_ref: A reference to an array that keeps track of buffer indices.
        step_ref: A reference to an array that tracks the step count within the attention computation.
        q_ref: A reference to the query array, [batch_size, num_heads, head_dim] array.
        k_pages_hbm_ref: A reference to the key pages stored in high-bandwidth memory,
            [num_kv_heads, total_num_pages, page_size, head_dim] array.
        v_pages_hbm_ref: A reference to the value pages stored in high-bandwidth memory,
            [num_kv_heads, total_num_pages, page_size, head_dim] array.
        o_ref: A reference to the output array to store attention outputs.
        m_ref: A reference to an array for storing intermediate maximum scores.
        l_ref: A reference to an array for storing intermediate normalization factors.
        k_vmem_buffer: A buffer for loading key pages into virtual memory.
        v_vmem_buffer: A buffer for loading value pages into virtual memory.
        sem: A semaphore for asynchronous copy operations.
        batch_size: Size of the batch.
        pages_per_compute_block: Number of pages processed in one block of attention computation.
        pages_per_sequence: Total number of pages per sequence (KV cache).
        mask_value: Value used for masking in attention.
        megacore_mode: Mode for megacore parallelism, one of ['kv_head', 'batch', None].
        program_ids: Tuple specifying program IDs for multi-program coordination.

    Notes:
        - This kernel uses multi-page asynchronous copy operations to load KV pages into memory.
        - Computation proceeds by performing attention over the loaded pages.
        - The results are accumulated across multiple steps to form the final attention output.
